...
from api.v1.response_models.auth import (
...
async def register_user(
    request: Request, data: UserRegistrationRequest
    ...
    logger.info(
        f"Auth Router: Registration attempt - Email: {data.email}, IP: {client_ip}"
    )
    ...
    try:
        ...
        if "jwt" in result:
            logger.info(
                f"Auth Router: Registration successful - Email: {data.email}, IP: {client_ip}"
            )
            ...
            logger.error(
                f"Auth Router: Registration failed - Email: {data.email}, Error: {error}, Message: {message}, IP: {client_ip}"
            )
            raise HTTPException(
        ...
        logger.error(
            f"Auth Router: Unexpected error during registration - Email: {data.email}, Error: {str(e)}, IP: {client_ip}"
        )
        raise HTTPException(
            status_code=status_codes.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=AuthService.format_error_response(
...
async def login_user(
    request: Request, data: UserLoginRequest
) -> AuthResponse | ErrorResponse:
    """
    ...
    try:
        result = await AuthService.login_user(email=data.email, password=data.password)

        if "jwt" in result:
            logger.info(
                f"Auth Router: Login successful - Email: {data.email}, IP: {client_ip}"
            )
            return AuthService.format_auth_response(result["jwt"])
        else:
            ...
            logger.warning(
                f"Auth Router: Login failed - Email: {data.email}, Error: {error}, Message: {message}, IP: {client_ip}"
            )
            raise HTTPException(
        ...
        logger.error(
            f"Auth Router: Unexpected error during login - Email: {data.email}, Error: {str(e)}, IP: {client_ip}"
        )
        raise HTTPException(
            status_code=status_codes.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=AuthService.format_error_response(
...
async def refresh_token(
    ...
    try:
        ...
        message = str(e)
        logger.error(
            f"Auth Router: Token refresh failed - Error: {error}, Message: {message}, IP: {client_ip}"
        )
        raise HTTPException(
            status_code=status_codes.HTTP_400_BAD_REQUEST,
            detail=AuthService.format_error_response(error, message).__repr__(),
        ...
        logger.error(
            f"Auth Router: Unexpected error during token refresh - Error: {str(e)}, IP: {client_ip}"
        )
        raise HTTPException(
            status_code=status_codes.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=AuthService.format_error_response(
...
async def reset_password(
    request: Request, data: ResetPasswordRequest
) -> bool | ErrorResponse:
    """
    ...
    client_ip = request.client.host if request.client else "unknown"
    logger.info(
        f"Auth Router: Password reset attempt - Email: {data.email}, IP: {client_ip}"
    )

    try:
        result = await AuthService.reset_password(
            jwt_token=data.jwt, email=data.email, password=data.password
        )

        if result:
            logger.info(
                f"Auth Router: Password reset successful - Email: {data.email}, IP: {client_ip}"
            )
            return result
        else:
            ...
            logger.error(
                f"Auth Router: Password reset failed - Email: {data.email}, Error: {error}, Message: {message}, IP: {client_ip}"
            )
            raise HTTPException(
                status_code=status_codes.HTTP_400_BAD_REQUEST,
                detail=AuthService.format_error_response(error, message).__repr__(),
        ...
        logger.error(
            f"Auth Router: Unexpected error during password reset - Email: {data.email}, Error: {str(e)}, IP: {client_ip}"
        )
        raise HTTPException(
            status_code=status_codes.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=AuthService.format_error_response(
...
router = Router(
    path="/auth",
    route_handlers=[register_user, login_user, refresh_token, reset_password],
)
